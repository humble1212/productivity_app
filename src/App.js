...
function NavigationBar(props) {
  ...
  return (
    <main className=" w-dvw h-dvh bg-teal-50 flex flex-col pb-1">
      {/* small screen */}
      <header className=" h-auto w-full shadow-lg p-2 flex items-center justify-between bg-teal-950 text-white sm:visible md:hidden lg:hidden xl:hidden 2xl:hidden">
        <GiHamburgerMenu
          onClick={() => {
            setSideBar(!sideBar);
          }}
      ...
      <section className=" w-full flex-1 flex relative">
        {sideBar && (
          <nav className="h-full w-60 flex flex-col py-2 gap-2 items-end  bg-teal-950 text-white sm:absolute left-0 bottom-0 top-0 z-10">
